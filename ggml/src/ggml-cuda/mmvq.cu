...
static __global__ void mul_mat_vec_q2_k_moe_reduce(
        const void * __restrict__ x, const block_q8_1 * __restrict__ y, const int32_t * __restrict__ ids,
        const float * __restrict__ scales, const float * __restrict__ weights, float * __restrict__ dst,
        int64_t nrows, int64_t nblocks, int64_t x_row_stride, int64_t x_expert_stride,
        int64_t y_row_stride, int64_t ids_stride, int n_used) {
    constexpr int rows = 2;
    constexpr int qi = ggml_cuda_type_traits<GGML_TYPE_Q2_K>::qi;
    constexpr int vdr = get_vdr_mmvq(GGML_TYPE_Q2_K);
    constexpr int blocks_per_iter = vdr * WARP_SIZE / qi;
    constexpr vec_dot_q_cuda_t dot = get_vec_dot_q_cuda(GGML_TYPE_Q2_K);
    const int lane = threadIdx.x;
    const int expert = threadIdx.y;
    const int64_t token = blockIdx.y;
    const int64_t row0 = int64_t(blockIdx.x) * rows;
    const int64_t assignment = token * n_used + expert;
    const int64_t x_offset = int64_t(ids[token * ids_stride + expert]) * x_expert_stride + row0 * x_row_stride;
    const block_q8_1 * activation = y + assignment * y_row_stride;
    float partial[rows] = {};

    for (int64_t kb = lane / (qi/vdr); kb < nblocks; kb += blocks_per_iter) {
#pragma unroll
        for (int row = 0; row < rows; ++row) {
            if (row0 + row < nrows) {
                partial[row] += dot(x, activation + kb * (QK_K/QK8_1), x_offset + row * x_row_stride + kb, vdr * (lane % (qi/vdr)));
            }
        }
    }
#pragma unroll
    for (int row = 0; row < rows; ++row) {
        partial[row] = warp_reduce_sum<WARP_SIZE>(partial[row]);
    }
    __shared__ float weighted[8][rows];
    if (lane < rows) {
        float value = partial[lane];
        if (scales) {
            value *= scales[assignment];
        }
        weighted[expert][lane] = value * weights[assignment];
    }
    __syncthreads();
    if (expert == 0 && lane < rows && row0 + lane < nrows) {
        float sum = weighted[0][lane];
        for (int e = 1; e < n_used; ++e) {
            sum += weighted[e][lane];
        }
        dst[token * nrows + row0 + lane] = sum;
    }
}

bool ggml_cuda_can_fuse_moe_down(const ggml_tensor * down, int cc) {
    if (!GGML_CUDA_CC_IS_NVIDIA(cc) || cc < GGML_CUDA_CC_BLACKWELL || cc >= GGML_CUDA_CC_RUBIN ||
            down->op != GGML_OP_MUL_MAT_ID || down->type != GGML_TYPE_F32 || !ggml_is_contiguous(down)) {
        return false;
    }
    const ggml_tensor * x = down->src[0];
    const ggml_tensor * y = down->src[1];
    const ggml_tensor * ids = down->src[2];
    return x && y && ids && x->type == GGML_TYPE_Q2_K && ggml_is_contiguous(x) && x->ne[3] == 1 &&
        x->ne[0] % QK_K == 0 && y->type == GGML_TYPE_F32 && y->nb[0] == sizeof(float) &&
        y->ne[0] == x->ne[0] && y->ne[1] == down->ne[1] && y->ne[2] == down->ne[2] && y->ne[3] == 1 &&
        ids->type == GGML_TYPE_I32 && ids->nb[0] == sizeof(int32_t) && ids->ne[0] == down->ne[1] &&
        ids->ne[1] == down->ne[2] && ids->ne[2] == 1 && ids->ne[3] == 1 &&
        down->ne[0] == x->ne[1] && down->ne[1] >= 2 && down->ne[1] <= 8 &&
        down->ne[2] >= 1 && down->ne[2] <= 4 && down->ne[3] == 1;
}

void ggml_cuda_moe_down_reduce(ggml_backend_cuda_context & ctx, const ggml_tensor * down,
        const ggml_tensor * expert_scale, const ggml_tensor * weights, ggml_tensor * dst) {
    GGML_ASSERT(ggml_cuda_can_fuse_moe_down(down, ggml_cuda_info().devices[ctx.device].cc));
    const ggml_tensor * x = down->src[0];
    const ggml_tensor * y = down->src[1];
    const ggml_tensor * ids = down->src[2];
    const int64_t padded = GGML_PAD(y->ne[0], MATRIX_ROW_PADDING);
    const int64_t stride = padded / QK8_1;
    ggml_cuda_pool_alloc<block_q8_1> packed(ctx.pool(), stride * y->ne[1] * y->ne[2]);
    quantize_row_q8_1_cuda((const float *) y->data, nullptr, packed.get(), x->type, y->ne[0],
        y->nb[1]/sizeof(float), y->nb[2]/sizeof(float), y->nb[3]/sizeof(float), padded,
        y->ne[1], y->ne[2], 1, ctx.stream());
    const dim3 blocks((down->ne[0] + 1)/2, down->ne[2]);
    const dim3 threads(WARP_SIZE, down->ne[1]);
    mul_mat_vec_q2_k_moe_reduce<<<blocks, threads, 0, ctx.stream()>>>(x->data, packed.get(), (const int32_t *) ids->data,
        expert_scale ? (const float *) expert_scale->data : nullptr, (const float *) weights->data, (float *) dst->data,
        x->ne[1], x->ne[0]/QK_K, x->nb[1]/sizeof(block_q2_K), x->nb[2]/sizeof(block_q2_K),
        stride, ids->nb[1]/sizeof(int32_t), down->ne[1]);
