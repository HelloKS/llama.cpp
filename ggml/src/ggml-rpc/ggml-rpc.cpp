...
struct rpc_msg_nccl_init_req {
    ...
    uint32_t rank;
    uint32_t world;
    uint32_t flags; // bit 0: F32 large reductions; bit 1: NCCL small reductions (interface v2)
    uint32_t id_size;
    uint8_t id[256];
};
...
public:
    ...
    void invalidate_graph_definitions();
...
private:
    ...
    struct graph_definition_cache {
        ...
        std::unordered_map<uint64_t, uint64_t> aliases;
        size_t bytes = 0;
    };
    std::unordered_map<uint32_t, graph_definition_cache> graph_definitions;
    rpc_msg_queue    queue;
    socket_ptr       sock;
...
static void ggml_backend_rpc_buffer_free_buffer(ggml_backend_buffer_t buffer) {
    ...
    ctx->dispatcher->invalidate_graph_definitions();
...
static std::vector<uint8_t> graph_definition(const uint8_t * input, size_t size) {
    std::vector<uint8_t> result(input, input + size);
    memset(result.data() + sizeof(uint32_t), 0, sizeof(uint64_t));
    uint32_t n_nodes;
    memcpy(&n_nodes, input + sizeof(uint32_t) + sizeof(uint64_t), sizeof(n_nodes));
    const size_t nodes_offset = 2*sizeof(uint32_t) + sizeof(uint64_t);
    const size_t count_offset = nodes_offset + n_nodes*sizeof(uint64_t);
    uint32_t n_tensors;
    memcpy(&n_tensors, input + count_offset, sizeof(n_tensors));
    const size_t tensors_offset = count_offset + sizeof(uint32_t);
    GGML_ASSERT(tensors_offset + n_tensors*sizeof(rpc_tensor) == size);

    // Tensor object addresses can change on rebuild; preserve graph connectivity instead.
    std::unordered_map<uint64_t, uint64_t> ids;
    ids.reserve(n_tensors + 1);
    ids.emplace(0, 0);
    for (uint32_t i = 0; i < n_tensors; ++i) {
        uint64_t id;
        memcpy(&id, input + tensors_offset + i*sizeof(rpc_tensor), sizeof(id));
        ids.emplace(id, i + 1);
    }
    for (uint32_t i = 0; i < n_nodes; ++i) {
        uint64_t id;
        memcpy(&id, input + nodes_offset + i*sizeof(id), sizeof(id));
        id = ids.at(id);
        memcpy(result.data() + nodes_offset + i*sizeof(id), &id, sizeof(id));
    }
    const auto canonical_id = [&](uint64_t id) { return ids.at(id); };
    for (uint32_t i = 0; i < n_tensors; ++i) {
        rpc_tensor tensor;
        memcpy(&tensor, input + tensors_offset + i*sizeof(tensor), sizeof(tensor));
        tensor.id = canonical_id(tensor.id);
        tensor.view_src = canonical_id(tensor.view_src);
        for (int j = 0; j < GGML_MAX_SRC; ++j) {
            tensor.src[j] = canonical_id(tensor.src[j]);
        }
        memcpy(result.data() + tensors_offset + i*sizeof(tensor), &tensor, sizeof(tensor));
    }
    return result;
}

void rpc_dispatcher::invalidate_graph_definitions() {
    std::lock_guard<std::mutex> lock(graph_mutex);
    // Freed buffer addresses can be reused by later allocations.
    graph_definitions.clear();
}
...
void rpc_dispatcher::graph_compute(uint32_t device, const ggml_cgraph * cgraph) {
    ...
    auto & cache = graph_definitions[device];
    const auto recompute = [&](uint64_t uid) {
        auto request = std::make_shared<rpc_msg_graph_recompute_req>();
        request->device = device;
        request->uid    = uid;
        send_async(RPC_CMD_GRAPH_RECOMPUTE, request, sizeof(*request));
    };
    if (cgraph->uid != 0 && device_graph_uids.count(cgraph->uid)) {
        recompute(cgraph->uid);
        return;
    }
    auto alias = cache.aliases.find(cgraph->uid);
    if (alias != cache.aliases.end()) {
        recompute(alias->second);
        return;
    }
    ...
    std::shared_ptr<uint8_t> input_ptr(input, std::default_delete<uint8_t[]>());
    static const bool match_definitions = [] {
        const char * value = std::getenv("GGML_RPC_GRAPH_CACHE");
    ...
    std::vector<uint8_t> definition;
    uint64_t hash = 0;
    if (match_definitions && cgraph->uid != 0 && input_size <= max_bytes) {
        definition = graph_definition(input, input_size);
        hash = fnv_hash(definition.data(), definition.size());
        auto it = std::find_if(cache.entries.begin(), cache.entries.end(), [&](const auto & entry) {
            return entry.hash == hash && entry.definition == definition;
        });
        if (it != cache.entries.end()) {
            if (cache.aliases.size() >= GRAPH_CACHE_MAX) {
                cache.aliases.clear();
            }
            cache.aliases.emplace(cgraph->uid, it->uid);
            LOG_DBG("[%s] device: %u, graph definition reused: %" PRIu64 " -> %" PRIu64 "\n", __func__, device, cgraph->uid, it->uid);
            recompute(it->uid);
            std::rotate(it, it + 1, cache.entries.end());
            return;
        }
    }
    if (cgraph->uid != 0) {
    ...
    if (!definition.empty()) {
        while (cache.entries.size() >= max_entries || cache.bytes + definition.size() > max_bytes) {
            cache.bytes -= cache.entries.front().definition.size();
            cache.entries.erase(cache.entries.begin());
        }
        cache.bytes += definition.size();
        cache.entries.push_back({cgraph->uid, hash, std::move(definition)});
    }
...
bool rpc_server::comm_allreduce(const rpc_msg_comm_allreduce_req & request) {
    ...
    if (state.nccl && t_dst->type == GGML_TYPE_F32 && (ne >= 32768 || state.nccl->small)) {
...
bool rpc_server::nccl_info(const rpc_msg_nccl_info_req & request, rpc_msg_nccl_info_rsp & response) {
    ...
    if (!api || (api->version != 1 && api->version != 2) || api->id_size > sizeof(response.id)) {
        return true;
    }
...
bool rpc_server::nccl_init(const rpc_msg_nccl_init_req & request, rpc_msg_comm_init_rsp & response) {
    response.ok = 0;
    if (request.device >= backends.size() || request.id_size > sizeof(request.id) || request.flags > 3) {
        return false;
    }
    const bool fp32 = (request.flags & 1) != 0;
    const bool small = (request.flags & 2) != 0;
    auto & state = comm_states[request.device];
    ...
    auto get = (ggml_backend_get_nccl_interface_t) ggml_backend_reg_get_proc_address(reg, "ggml_backend_get_nccl_interface");
    const auto * api = get ? get() : nullptr;
    if (!api || (api->version != 1 && api->version != 2) || (small && api->version < 2) || api->id_size != request.id_size) {
        return true;
    }
    void * context = api->init_rank(backend, request.id, request.id_size, request.rank, request.world, fp32);
    if (context) {
        state.nccl = std::shared_ptr<nccl_handle>(new nccl_handle{api, context, small});
        response.ok = 1;
        GGML_LOG_INFO("%s: rank %u NCCL %d ready (large: %s; small: %s)\n", __func__, request.rank,
            api->library_version, fp32 ? "fp32 allreduce" : "bf16 exchange", small ? "fp32 allreduce" : "pairwise");
    }
    return true;
}
...
static bool ggml_backend_rpc_nccl_init(const std::vector<ggml_backend_rpc_comm_context::rank_info> & ranks, bool fp32, bool small) {
    ...
    for (const auto & rank_info : info) {
        if ((rank_info.version != 1 && rank_info.version != 2) || (small && rank_info.version < 2) ||
            rank_info.id_size == 0 || rank_info.id_size > sizeof(rank_info.id) ||
            rank_info.id_size != info[0].id_size || rank_info.library_version != info[0].library_version) {
            GGML_LOG_WARN("%s: both workers need matching NCCL >= 2.18 support%s\n", __func__,
                small ? " and NCCL interface v2 for nccl-decode (rebuild both workers)" : "");
            return false;
        }
    }
    ...
    for (size_t i = 0; i < ranks.size(); i++) {
        ...
        request->flags = (fp32 ? 1u : 0u) | (small ? 2u : 0u);
    ...
    if (std::all_of(responses.begin(), responses.end(), [](const auto & response) { return response.ok != 0; })) {
        GGML_LOG_INFO("%s: NCCL %u %s enabled for reductions >= 32768 elements; small reductions use %s\n",
            __func__, info[0].library_version, fp32 ? "fp32 allreduce" : "bf16 exchange",
            small ? "NCCL fp32 allreduce" : "pairwise");
        return true;
    }
...
static void * ggml_backend_rpc_comm_init(ggml_backend_t * backends, size_t n_backends) {
    ...
    if (mode != "pairwise" && mode != "auto" && mode != "nccl-exchange" && mode != "nccl-f32" && mode != "nccl-decode") {
        GGML_ABORT("Unknown GGML_RPC_ALLREDUCE value: %s", mode.c_str());
    }
    auto unsupported = [&]() -> void * {
        if (mode == "nccl-exchange" || mode == "nccl-f32" || mode == "nccl-decode") {
    ...
    if (mode != "pairwise" && !ggml_backend_rpc_nccl_init(ranks, mode == "nccl-f32", mode == "nccl-decode")) {
        if (mode != "auto") {
